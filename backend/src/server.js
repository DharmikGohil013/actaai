...
app.post('/api/bot/setup/start', verifyToken, async (req, res) => {
    try {
        ...
        if (errMsg.includes('DISPLAY is not set') || errMsg.includes('Failed to launch the browser process')) {
            userFriendlyError = 'Google Account authentication is only supported in local development. Headless cloud servers cannot launch graphical setup windows. However, Google Meet supports guest joining automatically - NO account setup is required!';
        }
